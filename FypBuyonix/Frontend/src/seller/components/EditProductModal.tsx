...
interface EditProductModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateProduct: () => void; // Changed to just refresh
  product: any;
}

const EditProductModal = ({ isOpen, onClose, onUpdateProduct, product }: EditProductModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        onUpdateProduct(); // Refresh product list
        onClose();
      } else {
